...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			...
			$("#AddRow").click(function(){		
				var tr=$("#myTable tr").eq(0).clone();//找到第0行 克隆一行
				++i;
				var j=i-1;//表示报销单明细下标
				var item = $("#item").val()//项目
				var account = $("#account").val();//金额
				totalAccount=parseFloat(totalAccount)+parseFloat(account);
				var desc = $("#desc").val();//费用说明
		...
		function submitClaimVoucher(action){
	   		...
	   		if (action == '保存'){
	   			...
	   			alert(document.claimForm.status.value);
	   		}else{
	...
	<div class="action  divaction">
		...
		<div class="pages">
			<s:form action="addBizClaimVoucher.action" name="claimForm">
			...
			<input type="hidden" id="rowNumber" name="rowNumber" value="<s:property value="rowNumber"/>"/>
				<table width="90%" border="0" cellspacing="0" cellpadding="0" class="addform-base">
					...
					<tr>
						<td>*总金额：￥</td>
						<td><input type="text" id="totalAccount" name="claimVoucher.totalAccount" value="0" readonly="readonly"/></td>
						<td>*状态：</td>
